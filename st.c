...
typedef struct {
	...
	ImageList *images;     /* sixel images. Head is at the top layer. */
...
dcshandle(void)
{
	...
	default:
		...
		break;
	case 'q': /* DECSIXEL */
		if (sixel_parser_init(&sixel_st, getcolrgb(defaultfg), getcolrgb(defaultbg), 1, win.cw, win.ch) != 0)
...
draw(void)
{
	...
	term.ocx = cx;
	term.ocy = term.c.y;
        xdrawimages(term.images, term.row, term.col, term.line);
	cleanupimages();
	xfinishdraw();
